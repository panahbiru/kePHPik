...
Class Db
{
    public function __construct()
    {
        require_once "ezsql.php";
        $__db = new ezSQL_sqlite3( dirname(__FILE__).'/', 'store.db' );
        $this->db = $__db;
    }
}
...
function read_flatdb($table)
{
    $path = dirname(__FILE__).'/flatdb/';
    if(file_exists($path.$table.'.db'))
    {
        $data = file_get_contents($path.$table.'.db');
        return $data;
    }else{
...
function parse_flatdb($table)
{
    $path = dirname(__FILE__).'/flatdb/';
    if(file_exists($path.$table.'.db'))
    {
        $raw    = file_get_contents($path.$table.'.db');
        $data   = explode("\n", $raw);
        $cnt    = count($data) - 1;
        unset($data[$cnt]);
